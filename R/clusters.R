...
#' @export
annotate.profile.using.markers <- function (profile, markers, features_use = NULL) 
{
    features_use = rownames(profile)
...
#' @param initial_clustering Used as the inital clustering
#' @param seed Random seed
#'
#' @return ace with added annotation
#'
#' @examples
#' clusters <- clusterNetwork(G, 1.0)
#' @export
...
Leiden.clustering <- function(ace,
  ...
  clusters <- clusterNetwork(G, resolution_parameter, initial.clusters, seed)
  if (postprocess == T) {
    cc <- table(clusters)
